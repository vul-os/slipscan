...
pub fn spending_csv(rows: &[SpendingRow]) -> String {
    let mut out = row(&["category_id", "category_name", "total_minor"]);
    for r in rows {
        out.push_str(&row(&[
...
pub fn spending_by_month_csv(rows: &[MonthlySpendingRow]) -> String {
    let mut out = row(&["month", "category_id", "category_name", "total_minor"]);
    for r in rows {
        out.push_str(&row(&[
            &r.month,
            r.category_id.as_deref().unwrap_or(""),
...
pub fn trial_balance_csv(rows: &[TrialBalanceRow]) -> String {
    let mut out = row(&["code", "name", "kind", "debit_minor", "credit_minor"]);
    for r in rows {
        out.push_str(&row(&[
            &r.code,
            &r.name,
...
pub fn income_statement_csv(statement: &IncomeStatement) -> String {
    let mut out = row(&["section", "code", "name", "amount_minor"]);
...
mod tests {
    ...
    fn spending_csv_has_header_and_rows() {
        ...
        assert_eq!(lines.next(), Some("category_id,category_name,total_minor"));
        assert_eq!(lines.next(), Some("cat-1,\"Groceries, snacks\",12345"));
        assert_eq!(lines.next(), None);
    }

    #[test]
    fn trial_balance_csv_renders_kinds() {
        ...
        assert!(csv.contains("1000,Bank,asset,100,0\n"));
    }
}
