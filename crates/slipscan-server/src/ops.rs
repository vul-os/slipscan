...
//! the HTTP routes and the CLI: signed pack installation and pack sources.
//!
//! The date-ranged ledger reports (income statement, balance sheet, tax
//! summary) used to be reimplemented here as ALL-TIME duplicates
//! (`report_profit_loss`, `report_balance_sheet`, `report_tax`) of core's own
//! date-aware `CoreService::report_income_statement` /
//! `report_balance_sheet` / `report_tax_summary`. That second implementation
//! is gone — every surface (HTTP, CLI, desktop) now calls core directly, so
//! there is exactly one accounting engine and every report can be scoped to
//! a period.
...
use slipscan_core::domain::CategoryNode;
...
mod tests {
    ...
    use slipscan_core::domain::{
        AccountKind, BookKind, CategoryKind, NewAccount, NewBook, NewCategory, NewTransaction,
        TransactionSource,
    };
