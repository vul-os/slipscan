...
//
// Usage:
//   node scripts/check-reachable.ts            report, exit 1 if worse than the baseline
//   node scripts/check-reachable.ts --list     print the full reachability table
//   node scripts/check-reachable.ts --write    record the current counts as the baseline
...
const table: ReachabilityRow[] = methods.map((name) => {
...
const current: Omit<ReachabilityBaseline, "//"> = {
    ...
    `${JSON.stringify(
      {
        "//": [
          ...
          "can reach. Generated by scripts/check-reachable.ts --write.",
          "",
...
const baseline = JSON.parse(read(BASELINE)) as ReachabilityBaseline;
...
if (fixed.length) {
  ...
  console.log(`  re-record the baseline: node scripts/check-reachable.ts --write`);
}
...
if (fixed.length) {
  console.error(
    `\ncheck-reachable: the baseline is stale — ${fixed.length} method(s) are now reachable.\n` +
      `  Re-record it so the ratchet tightens: node scripts/check-reachable.ts --write`,
  );
  process.exit(1);
}
