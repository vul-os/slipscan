...
//   node scripts/check-mock-guards.mjs --write   re-record the baseline
...
function coreGuards() {
  ...
  const starts = [...src.matchAll(/^ {4}pub fn (\w+)/gm)].map((m) => ({
    name: m[1],
    at: m.index,
  }));
...
function mockGuards() {
  ...
  const starts = [...src.matchAll(/^ {2}(\w+): async \(/gm)].map((m) => ({
    name: m[1],
    at: m.index,
  }));
  if (starts.length < 80) {
...
// no mock is a separate question — `check-parity.mjs` owns that one.
const rows = [...mock.keys()]
...
const current = Object.fromEntries(rows.map((r) => [r.name, [r.core, r.mock]]));
    ...
    `${JSON.stringify(
      {
        "//": [
          ...
          "Generated by scripts/check-mock-guards.mjs --write.",
          "",
...
const baseline = JSON.parse(read(BASELINE)).operations ?? {};
const problems = [];
