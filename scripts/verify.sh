#!/usr/bin/env bash
# =============================================================================
# scripts/verify.sh — verify a downloaded FlowStock release artifact, fail-closed.
#
...
# WHAT IT IS FOR
# ──────────────
# A FlowStock release publishes one tar.gz per platform plus a `SHA256SUMS.txt`
...
# shrugs at a 404 on SHA256SUMS.txt prints a line that looks like verification
...
#     name as a regex — every "." in a name like "flowstock_v0.3.0_darwin_arm64.tar.gz" is a wildcard
...
#   3   SHA256SUMS.txt could not be fetched / does not exist (HTTP error, DNS,
#       refused connection, or absent from a local --dir)
#   4   SHA256SUMS.txt was served, but the body is an HTML page, not a manifest
#       (captive portal, CDN error page, a 200 that means "no such key")
#   5   SHA256SUMS.txt is empty, or contains no well-formed "<64 hex>  <name>" line
#   6   SHA256SUMS.txt is valid but has no entry for the artifact requested
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.3.0 flowstock_v0.3.0_darwin_arm64.tar.gz
#   scripts/verify.sh --tag v0.3.0 --attest flowstock_v0.3.0_darwin_arm64.tar.gz
#   scripts/verify.sh --repo vul-os/flowstock --tag v0.3.0 --out ~/Downloads ASSET
...
SELFTEST_ARTIFACT="flowstock_v9.9.9_linux_amd64.tar.gz"
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/flowstock-verify.XXXXXX")"
...
run_selftest() {
  ...
  expect_exit "--attest but no gh installed"  "$E_ATTEST"         yes -- env PATH="$fakebin" "$bash_abs" "$SELF_PATH" --out "$outd" --attest --repo vul-os/flowstock --base-url "$base/good" "$art"
