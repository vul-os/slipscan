...
- The set of entities was taken from the schema, not from prose: the tables created in
  ...
  SlipScan can store, and there is no `invoices`, `bills`, `contacts`, `fixed_assets`,
  `bank_rules`, `tracking_categories`, or `goals` among them.
