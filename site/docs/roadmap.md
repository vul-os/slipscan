...
- [ ] Import/export: CSV, OFX, and migration from the legacy Cloudflare stack *(partial: statement import is wired for both CSV and OFX on the CLI, and the CSV/preset half is now wired on desktop and HTTP too. `slipscan import <file> --preset <id> --account <acct>` parses CSV rows through the region-grouped preset catalog (`--list-presets`). `slipscan import <file.ofx> --account <acct>` parses OFX 1.x (SGML) and OFX 2.x (XML) statements directly — no preset needed, since OFX carries its own structure — reading `DTPOSTED`/`TRNAMT`/`NAME`/`MEMO`/`FITID` per transaction plus `CURDEF` and `LEDGERBAL`. Every path calls the identical `import_statement_lines` core function, so dedupe (by FITID for OFX, content-hash otherwise), the categorisation cascade and the Payments hook all apply without a second implementation; a malformed file or a `<STMTTRN>` missing its date/amount/description refuses with a specific reason rather than guessing. The desktop's Transactions screen ("Import statement") and the server's `statement_preset_list`/`statement_import` routes now drive the CSV/preset half end to end — pick a preset and an account, hand over a file, see what imported and what deduped — through the same function the CLI calls, not a second one. **OFX is still CLI-only**: the desktop and HTTP surfaces store an `.ofx` file as a document without parsing it, and a fully custom CSV column mapping has no picker on any surface yet (the library supports it; only `slipscan import`'s flags don't expose it). `slipscan watch <dir>` imports a drop folder as documents. Trial-balance CSV export and desktop report exports exist; migration from the legacy Cloudflare stack does not)*
...
- [ ] Slip/receipt capture: drag-drop, file watch, camera (mobile later) *(partial: drag-and-drop and a watch-folder setting now ship in the desktop app, alongside the file picker that already worked. Drag-and-drop lands anywhere in the app window — Tauri's webview file-drop event hands over real paths (`document_import`'s existing `path` field, not `bytes_base64`), with a DOM drag-and-drop fallback outside Tauri (plain browser dev, every Playwright spec); multiple files at once, and an unsupported type is rejected with the reason `document_import` itself gives, off `crates/slipscan-ingest/src/import.rs`'s accepted-extension list — no second copy of that list lives in the frontend. The file picker remains, so neither drag-and-drop nor the keyboard is the only way in. The watch-folder setting (Settings › Connections) wires the same engine `slipscan watch <dir>` runs on the CLI into the app, honestly scoped: it only watches while SlipScan is open — a background thread using the OS's own filesystem-event API, no polling, no network — resumes on its own the next launch if left on, and stops the moment the app closes; there is no separate always-on process. `slipscan watch <dir> --once` remains the answer for cron/launchd. Camera capture is not built — it needs a mobile decision that has not been made)*
- [x] Fully offline operation

## Phase 3 — Ingestion: your bank, your inbox

- [ ] **Bank scraper framework**: one open-source adapter per bank type, sandboxed, credentials stored in the OS keychain, sessions run locally *(partial: `BankAdapter` trait + statement pipeline + region CSV presets ship, and the statement path is now wired on all three surfaces — CLI (`slipscan import --preset`), desktop (Transactions' "Import statement" dialog: pick a preset, pick the account, see what imported and what deduped) and HTTP (`statement_preset_list`/`statement_import`) — every one of them through the identical `import_statement_lines` core function, so dedupe, the categorisation cascade and the Payments hook apply the same way regardless of surface. The only non-test `BankAdapter` implementation still reads files: nothing talks to a bank, nothing runs on a schedule, and no credential ever reaches an adapter)*
