...
Suppliers and locations referenced below are now creatable on every surface — [contacts](#contacts-phase-62) got theirs after this section was first written, and locations always had them. **Product variants still do not**: none of the sixteen catalogue operations is routed here, on the CLI, or over IPC, so a PO line's `variant_id` can only name a row something else created. That is the remaining half of a gap this section used to describe in full, and `npm run reachable:check` is what now keeps score of it.
