...
Yes — day one. Global-by-default is a binding rule of the architecture: **no jurisdiction is hardcoded into core; everything country-specific ships as a region profile** — data you pick, never an assumption baked into code ([ARCHITECTURE.md](ARCHITECTURE.md#global-by-default--regions-are-data-not-code)). A region profile carries chart-of-accounts seeds, tax configuration (rates plus what your tax return is called and how it maps), bank statement CSV presets, and regional merchant packs. South Africa is simply the **first region profile** — fully supported, never special-cased ("VAT201" is the SA profile's label for the generic tax-period summary, not a core concept). A **generic profile** — neutral chart of accounts, a configurable tax rate, custom CSV column mapping, and the global `intl-starter` classification pack — covers any country before its dedicated profile exists. Money is currency-agnostic throughout (`i64` minor units + ISO-4217 code, exponent-aware, so JPY and BHD are as at home as ZAR or EUR), and multi-currency FX is available by pointing SlipScan at an [OpenRate](https://github.com/vul-os/openrate) instance you host or trust — opt-in, with provenance-graded rates ([CONFIGURATION.md](CONFIGURATION.md)). The globalization pass is landing phase-by-phase (Phase 4.7 in [ROADMAP.md](../ROADMAP.md)) — anything still SA-shaped in the current build is a bug against this contract, not a design choice.
