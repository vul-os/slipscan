...
- The **HTTP server is the canonical, near-complete surface** — the operation tables below describe it. 190 routes under `/api/v1` (188 distinct handlers: `report_vat` is a deprecated alias of `report_tax`, and `report_profit_loss` is a deprecated alias of `report_income_statement`), plus the public `GET /health`.
- The **desktop IPC exposes a UI-shaped subset** — **181 commands** with display-oriented DTOs (`health` in `src-tauri/src/lib.rs`, the other 180 in `src-tauri/src/commands.rs`). `apps/desktop/src/lib/api/client.ts` calls **all 181**: every registered command has a typed wrapper, and [`npm run parity:check`](../scripts/check-parity.ts) fails CI if one ever does not. (Several were wired ahead of the screen that will call them, which is [ROADMAP.md](../ROADMAP.md) 6.9 rather than the phase that added them: `pack_install_seeds` was registered-but-uncalled until the Packs screen surfaced it, `pack_benchmark` had no IPC command until the same change added one, `book_create` had none until first-run setup needed to create the first book, the fourteen `device_*` commands had none until Settings › Devices surfaced identity and pairing, `book_profile`/`book_set_kind`/`book_set_multi_location_override`/`location_create`/`location_list`/`location_update`/`location_delete` had none until first-run setup and Settings needed to resolve and change a book's profile — see [Book profiles](#book-profiles-phase-60) below — and the eighteen `po_*` [purchasing](#purchasing-phase-64) and twenty-one [sales & invoicing](#sales-orders--invoicing-phase-65) operations are wired the same way. `report_income_statement` and `report_balance_sheet` were the same story once — see [Reports](#reports) — until both retired the ALL-TIME duplicates that used to compute them and gained real IPC commands. Period close (`close_period_check`/`close_period`/`reopen_period` — see [Period close](#period-close) below) landed on all three surfaces in the same change that added it, rather than in a later wiring pass — as did statement import (`statement_preset_list`/`statement_import`, ROADMAP.md Phase 3/4.95): the desktop's Transactions screen and the HTTP routes both call the identical `slipscan_ingest::bank::import_statement_lines` the CLI's `slipscan import --preset` always has, rather than a second implementation.) **19 HTTP operations have no IPC command**: `book_get`, `account_create`/`account_get`/`account_update`/`account_delete` (`account_list` **is** wired — Dashboard, Transactions, Reports and Settings all call it), `transaction_create`/`transaction_get`, `category_create`, `member_get`, `location_get` (create/list/update/delete **are** wired — first-run setup and Settings both call them), `budget_status`, the document status-machine ops (`document_transition`, `document_record_extraction`, `document_current_extraction`), `journal_get`, `coa_seed`, `report_spending_by_month`, `audit_list`, and `pack_source_publish` (publishing a pack into a shared folder is a CLI/self-host action; the desktop reads sources and installs from them, and has no publish screen).
