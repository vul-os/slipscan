...
- The **HTTP server is the canonical, near-complete surface** — the operation tables below describe it. 204 routes under `/api/v1` (202 distinct handlers: `report_vat` is a deprecated alias of `report_tax`, and `report_profit_loss` is a deprecated alias of `report_income_statement`), plus the public `GET /health`.
- The **desktop IPC exposes a UI-shaped subset** — **195 commands** with display-oriented DTOs (`health` in `src-tauri/src/lib.rs`, the other 194 in `src-tauri/src/commands.rs`). `apps/desktop/src/lib/api/client.ts` calls **all 195**: every registered command has a typed wrapper, and [`npm run parity:check`](../scripts/check-parity.ts) fails CI if one ever does not. (Several were wired ahead of the screen that will call them, which is [ROADMAP.md](../ROADMAP.md) 6.9 rather than the phase that added them: `pack_install_seeds` was registered-but-uncalled until the Packs screen surfaced it, `pack_benchmark` had no IPC command until the same change added one, `book_create` had none until first-run setup needed to create the first book, the fourteen `device_*` commands had none until Settings › Devices surfaced identity and pairing, `book_profile`/`book_set_kind`/`book_set_multi_location_override`/`location_create`/`location_list`/`location_update`/`location_delete` had none until first-run setup and Settings needed to resolve and change a book's profile — see [Book profiles](#book-profiles-phase-60) below — and the eighteen `po_*` [purchasing](#purchasing-phase-64), twenty-one `sales_order_*`/`invoice_*` and fourteen `quote_*` [quotes, sales orders & invoicing](#quotes-sales-orders--invoicing-phase-65) operations are wired the same way. `report_income_statement` and `report_balance_sheet` were the same story once — see [Reports](#reports) — until both retired the ALL-TIME duplicates that used to compute them and gained real IPC commands. Period close (`close_period_check`/`close_period`/`reopen_period` — see [Period close](#period-close) below) landed on all three surfaces in the same change that added it, rather than in a later wiring pass — as did statement import (`statement_preset_list`/`statement_import`, ROADMAP.md Phase 3/4.95): the desktop's Transactions screen and the HTTP routes both call the identical `slipscan_ingest::bank::import_statement_lines` the CLI's `slipscan import --preset` always has, rather than a second implementation.) **19 HTTP operations have no IPC command**: `book_get`, `account_create`/`account_get`/`account_update`/`account_delete` (`account_list` **is** wired — Dashboard, Transactions, Reports and Settings all call it), `transaction_create`/`transaction_get`, `category_create`, `member_get`, `location_get` (create/list/update/delete **are** wired — first-run setup and Settings both call them), `budget_status`, the document status-machine ops (`document_transition`, `document_record_extraction`, `document_current_extraction`), `journal_get`, `coa_seed`, `report_spending_by_month`, `audit_list`, and `pack_source_publish` (publishing a pack into a shared folder is a CLI/self-host action; the desktop reads sources and installs from them, and has no publish screen).
- Three names currently diverge where both surfaces exist: desktop `report_vat_summary` vs server `report_tax` (the server keeps `report_vat` as a deprecated route alias), desktop `ledger_account_list` vs server `coa_list`, and desktop `category_list` vs server `category_tree`. `settings_get`/`settings_set` share names across both, but the payloads diverge: the desktop carries a whole-settings UI blob, the server generic key/value pairs. `book_set_kind`/`book_set_multi_location_override` share names too, but **their payloads diverge on purpose**: the HTTP route returns the updated `Book`, the desktop IPC command returns the resolved `BookProfile` directly, since that is what the Settings screen redraws from in one round trip. Discounting the three renames, **11 commands are desktop-only**: `journal_list`, `budget_list`, `report_income_expense`, `data_move`, `vault_set`/`vault_replace`, `pay_deliver_due`, `pack_verify`, `device_invite_cancel`, and `watch_folder_status`/`watch_folder_set`. Each omission is deliberate, not a gap — vault writes and the data-folder move are local-only by design (see [Data folder](#data-folder) and [What is deliberately absent](#what-is-deliberately-absent)), the server flushes webhook deliveries with its own loop instead of a route (see [Payments](#payments)), pack verification is a preflight the CLI does inline (`slipscan pack verify`), withdrawing a pairing invite belongs with the rest of the local-only invite lifecycle (`slipscan device cancel-invite`), and drop-folder watching only runs while the desktop process is open, which is a desktop-only concept with no server equivalent. `report_income_expense` in particular is a UI-only trend chart over raw transactions, deliberately **not** scoped by a report period — the account-level, period-scoped P&L is `report_income_statement`, which exists on every surface. The rest are UI conveniences the server has no reason to grow.
...
The surface, grouped by domain module. It mirrors the `pub fn`s on the core service, with one route name that differs from the core fn it calls: `report_tax` → core `report_tax_summary`. `report_vat` and `report_profit_loss` are kept as deprecated route aliases — `report_vat` for `report_tax`, `report_profit_loss` for `report_income_statement` (accounting's colloquial "profit & loss" for what core itself calls the income statement).
...
### Quotes, sales orders & invoicing (Phase 6.5)

PARITY.md's single largest Xero-axis gap: "there is no invoicing at all — no invoice entity, no numbering, no delivery, no paid/unpaid state." Closed by three tables across two different sync mappings — see migration `0014_sales`'s header. `quotes`/`quote_items` and `sales_orders`/`sales_order_items` are both editable drafts (a quote: draft → sent → accepted | declined | expired; an order: draft → confirmed → paid, or → cancelled); `invoices`/`invoice_items`/`invoice_payments` are only ever created, never edited — an issued invoice is a fact, not a form. All 35 operations exist under the same name on HTTP and desktop IPC; on the CLI they are `slipscan quote <action>`, `slipscan sales-order <action>` and `slipscan invoice <action>` (`slipscan report aged-receivables` for the last row). **These were the first Phase 6 tables with a real surface** — contacts (6.2), the catalogue (6.3a) and the stock ledger (6.3b) all shipped core-only at first, before being wired later in the same phase.

A quote is a priced offer that has not happened yet — it never touches `stock_movements` or `journals`, and has no `location_id` at all. **Accepting one (`quote_accept`) converts it into a sales order** by copying its lines into a brand-new `draft` order, reusing the exact `sales_orders`/`sales_order_items` rows and the same `repo::sales` functions `invoice_issue` already uses to copy a confirmed order's lines — not a second, parallel "confirm"-shaped path. The quote's own rows are never mutated by acceptance; `converted_sales_order_id` is the only trace left pointing at what they became. Deliberately out of scope: no e-mailing a quote, no PDF, no customer portal, and no partial acceptance — accepting copies every line or none.

| Operation | Purpose |
|---|---|
| `quote_create` / `quote_get` / `quote_list` / `quote_update` / `quote_delete` | A priced offer: contact, currency, optional expiry date, notes. `update` only reaches a `draft` quote (date/expiry/notes — never `status`); `delete` only reaches a `draft` quote too — decline a sent one instead |
| `quote_item_add` / `quote_items_list` / `quote_item_update` / `quote_item_remove` | Line items, identical shape to a sales order's: a catalogue line (`variant_id`, description/price default from the variant) or a free-text/service line. All three writes are refused once the quote leaves `draft` |
| `quote_send` | draft → sent. Requires at least one line item — an empty quote has nothing to offer |
| `quote_decline` | sent → declined. The customer said no; the quote stays on record |
| `quote_expire` | sent → expired. A deliberate call, not a background timer — nothing here watches `expiry_date`'s clock |
| `quote_accept` | sent → accepted, **and returns the new `SalesOrder`** rather than the `Quote` — the whole point of this operation. Copies this quote's lines into a brand-new draft order (`location_id: null`, same as any freshly created order) |
| `quote_totals` | `subtotal`/`tax`/`total`, derived from the quote's own lines at call time — never a stored column, identical rule to `sales_order_totals` below |
...
| `book_set_lock_date` | Set or clear the book's financial lock date directly — journals may not be posted on or before it. Most callers use `close_period` instead, which runs the checks below and moves this same field on success; this operation remains for setting the date with no checks at all (e.g. seeding a lock date on an imported book) |

### Period close

The ritual that turns a ledger into a book someone will sign — the last structural gap before ROADMAP.md Phase 6 could be called done. `close_period_check` and `close_period` run the **identical checks**: a hard refusal if the trial balance does not balance as of the close date (a data-integrity guard — every posted journal already balances on its own, so this only fires if something reached `journal_lines` outside the service layer) or if the range is already closed (re-closing the same or an earlier range is refused, not a silent no-op); and four advisory warnings that are always reported and **never block** — uncategorised transactions, unreconciled statement lines (no `auto`- or human-confirmed match), draft sales orders, and invoices due with nothing paid against them, all scoped to the period between the book's previous lock date and the new one. `close_period_check` never mutates; `close_period` advances `financial_lock_date` only when nothing hard-refuses. Both return the same `ClosePeriodReport` shape, differing only in `closed`.

| Operation | Purpose |
|---|---|
| `close_period_check` | Preview closing the book through `to_date` (inclusive) — every check `close_period` runs, with no mutation whatsoever. "What would closing this period tell me?", answerable any number of times |
| `close_period` | Close the book through `to_date`. Advances `financial_lock_date` on success (postings on or before it are refused from then on, the same guarantee `book_set_lock_date` already gives); refuses with every blocking reason named (422 `close_blocked`) otherwise. A close that succeeds still returns every advisory warning it found — closing does not make the messiness disappear, only seals the period it was found in |
| `reopen_period` | Reopen a closed period — a deliberate, reasoned, audited act, never a side effect. `reason` is required. `to_date` moves the lock date back to an earlier seal (a partial reopen) or omits it to clear the lock entirely; refuses (404 `not_closed`) if the book was never closed |
...
| Operation | Purpose |
|---|---|
| `report_spending` / `report_spending_by_month` | Spending breakdowns by category over a period; the `_by_month` form also groups by calendar month. **`report_spending_by_month` is HTTP + CLI (`slipscan report spending`) only — no desktop IPC command yet** |
| `report_trial_balance` | Trial balance for business books — every posted entry, no period (see [What is deliberately absent](#what-is-deliberately-absent) for why the derived reports below all take one and this one does not) |
| `report_income_statement` (alias `report_profit_loss`) / `report_balance_sheet` / `report_tax` (alias `report_vat`) | Income statement over an inclusive `from_date`/`to_date`, balance sheet as of an inclusive `as_of_date`, and the tax-period summary over `from_date`/`to_date` (base-currency) — labeled from the book's region profile ("VAT201" is the `za` profile's name for `report_tax`). **All three used to be reimplemented in `slipscan-server/src/ops.rs` as ALL-TIME duplicates with no period at all** (`report_profit_loss`/`report_balance_sheet`/`report_tax` computed over the book's entire history); that second implementation is gone, every surface now calls core's own date-aware `report_income_statement`/`report_balance_sheet`/`report_tax_summary` directly, and an inverted range (`from_date` after `to_date`) is refused rather than silently answered empty |
