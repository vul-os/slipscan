...
- [ ] Import/export: CSV, OFX, and migration from the legacy Cloudflare stack *(partial: statement import is wired for both CSV and OFX. `slipscan import <file> --preset <id> --account <acct>` parses CSV rows through the region-grouped preset catalog (`--list-presets`). `slipscan import <file.ofx> --account <acct>` parses OFX 1.x (SGML) and OFX 2.x (XML) statements directly — no preset needed, since OFX carries its own structure — reading `DTPOSTED`/`TRNAMT`/`NAME`/`MEMO`/`FITID` per transaction plus `CURDEF` and `LEDGERBAL`. Both paths call the same `import_statement_lines` core function, so dedupe (by FITID for OFX, content-hash otherwise), the categorisation cascade and the Payments hook all apply without a second implementation; a malformed file or a `<STMTTRN>` missing its date/amount/description refuses with a specific reason rather than guessing. `slipscan watch <dir>` imports a drop folder as documents. All three are CLI-only: the desktop stores a file without parsing it, CSV or OFX alike, and there is no bank-scraper OFX download automation. Trial-balance CSV export and desktop report exports exist; migration from the legacy Cloudflare stack does not)*
