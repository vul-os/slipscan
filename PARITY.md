...
honest — **4 of 175 core operations reachable from nothing, down from 42** (down from 14 as of this
...
|---|---|---|---|
| **Invoicing** | **Built**<!-- parity reachable="invoice_issue,invoice_payment_record" --> | `invoices` / `invoice_items` / `invoice_payments` (migration `0014_sales`), issued through `CoreService::invoice_issue` with gapless per-book numbering, and reachable over HTTP, the CLI and desktop IPC. Since ROADMAP 6.6, `invoice_payment_record` posts **DR** bank / **CR** accounts-receivable in the same transaction as the payment fact (migration `0016_ledger_trade`) | An issued invoice is immutable by trigger, so correcting one needs a credit note — **not built**, along with voiding, quotes, repeating invoices, partial fulfilment, and per-line currency. Multi-device numbering is unsolved: two offline devices would both mint the same number, caught loudly by `UNIQUE (book_id, series, number)` rather than silently. **Re-scored 2026-08-05**: both caveats this row used to carry are closed — the catalogue closed on 2026-08-03 (`product_variant_add` is reachable on all three surfaces, see `npm run reachable:check`), so a stock-tracked line **is** exercisable now too, and the 6.9 desktop screens landed [`desktop/src/routes/Sales.svelte`](apps/desktop/src/routes/Sales.svelte) with an Orders tab that issues straight into an invoice and an Invoices tab for standalone ones, both letting a line pick a variant. |
| **Quotes / estimates** | **Built**<!-- parity reachable="quote_create,quote_accept" --> | Quotes with lines, VAT and a `draft → sent → accepted | declined | expired` lifecycle in [`core/src/repo/sales.rs`](crates/slipscan-core/src/repo/sales.rs) (migration `0014_sales`). A quote touches neither stock nor the ledger — nothing is owed until it is accepted — and `quote_accept` converts it by copying its lines into a new draft sales order, so everything after that point is the existing order/invoice path rather than a parallel one. Reachable on CLI, HTTP, desktop IPC and the Sales screen's Quotes tab ([`desktop/src/routes/sales/Quotes.svelte`](apps/desktop/src/routes/sales/Quotes.svelte)) | **Re-scored 2026-08-05**, on the gate's prompt rather than a person noticing. No emailing a quote, no PDF, no customer-facing acceptance link, and no partial acceptance — accepting converts the whole quote or none of it. |
...
| **Fixed-asset register** | **Built**<!-- parity reachable="asset_create,depreciation_run,asset_dispose" --> | Assets with cost, residual, useful life and method in [`core/src/repo/assets.rs`](crates/slipscan-core/src/repo/assets.rs) (migration `0016_assets`); straight-line and reducing-balance schedules computed in exact `i64` minor units with no float anywhere; `depreciation_run` posts DR Depreciation / CR Accumulated depreciation through the same `post_journal_in_tx` path Phase 6.6 uses, so it is idempotent per (asset, period). Reachable on CLI, HTTP, desktop IPC and the Assets screen ([`desktop/src/routes/Assets.svelte`](apps/desktop/src/routes/Assets.svelte)) | **Re-scored 2026-08-05**, and the gate caught the stale score rather than a person noticing. No revaluation, no separate tax-vs-book depreciation, and no asset classes beyond the two methods. Disposal reverses posted depreciation rather than deleting it, because the ledger is immutable — there is no way to "undo" a run, only to reverse it. |
...
| **Repeating invoices / recurring transactions** | **Not built**<!-- parity absent="invoice_recurring_create,invoice_schedule_create" --> | — | No schedule, template or generator; the only "recurring" code is a read-only detector of recurring charges in [`desktop/src/lib/nudges.ts`](apps/desktop/src/lib/nudges.ts), which creates nothing. |
...
| **Payroll-lite** | **Not built**<!-- parity absent="payroll_run,employee_create" --> | `PAYE & UIF Payable` / `Payroll Taxes Payable` are chart-of-accounts seed lines in [`core/src/region.rs`](crates/slipscan-core/src/region.rs) | Everything: no employees, pay runs, payslips or statutory calculation. |
| **Tracking categories** | **Not built**<!-- parity absent="tracking_category_create" --> | Nearest analogue is the household-member dimension — `transaction_attribute`, `transaction_split_set`, `report_member_category` in [`core/src/service.rs`](crates/slipscan-core/src/service.rs) | That dimension is *people*, fixed and singular: no user-defined dimensions, nothing on journal lines, and no report sliceable by one. |
| **CSV / OFX import & export** | **Partial** | Import: [`ingest/src/bank/csv_statement.rs`](crates/slipscan-ingest/src/bank/csv_statement.rs) and per-region presets in [`ingest/src/bank/presets.rs`](crates/slipscan-ingest/src/bank/presets.rs), wired to `slipscan import --preset` in [`cli/src/main.rs`](crates/slipscan-cli/src/main.rs); OFX 1.x/2.x import in [`ingest/src/bank/ofx_statement.rs`](crates/slipscan-ingest/src/bank/ofx_statement.rs), wired to `slipscan import <file.ofx> --account <acct>` (no preset — OFX carries its own structure), through the same `import_statement_lines` core path CSV uses. Export: report CSVs in [`core/src/csv.rs`](crates/slipscan-core/src/csv.rs) | **Re-scored 2026-08-05**: desktop `statement_import`/`statement_preset_list` ([`desktop/src-tauri/src/commands.rs`](apps/desktop/src-tauri/src/commands.rs)) now exist as IPC commands with a `client.ts` wrapper — the desktop equivalent of `slipscan import --preset`, parsing CSV and OFX alike through the same `import_statement_lines` path — and the Transactions screen's "Import statement" dialog calls both ([`desktop/src/routes/Transactions.svelte`](apps/desktop/src/routes/Transactions.svelte)), so a statement import is reachable from the app; `document_import` remains a separate, screen-reachable action that only stores the file. No bank-adapter automates the OFX/CSV *download* itself; export is report-shaped only — [`desktop/src/lib/util/csv.ts`](apps/desktop/src/lib/util/csv.ts) is used by the Reports and Receipts screens and nowhere else, so there is no transaction-list, ledger or whole-book export, OFX or CSV. |
...
| Capability | Status | Evidence | What's missing |
|---|---|---|---|
| **Account aggregation** | **Partial** | Accounts in [`core/src/repo/account.rs`](crates/slipscan-core/src/repo/account.rs); statement CSV import with bank presets in [`ingest/src/bank/`](crates/slipscan-ingest/src/bank); mailbox sync over IMAP, Gmail and Graph in [`ingest/src/email/`](crates/slipscan-ingest/src/email); bank-alert emails → transactions via signed `mailrules` packs ([`ingest/src/email/alerts.rs`](crates/slipscan-ingest/src/email/alerts.rs), [`packs/src/mailrules.rs`](crates/slipscan-packs/src/mailrules.rs)); the only non-test `BankAdapter` implementation reads files ([`ingest/src/bank/csv_statement.rs`](crates/slipscan-ingest/src/bank/csv_statement.rs)) | **Nothing connects to a bank**: no adapter talks to a bank API or feed, nothing runs on a schedule. The alert parser still runs from the CLI only (`slipscan mail-sync --alerts`); statement import is reachable from the Transactions screen's "Import statement" dialog as well as the CLI (see the CSV/OFX row). |
| **Net worth over time** | **Built**<!-- parity reachable="networth_capture,networth_backfill,networth_series" --> | `networth_snapshots` (migration `0015_networth`) records one immutable balance snapshot per account per date; `networth_capture` stamps today, `networth_backfill` reconstructs history from the transaction ledger, and `networth_series` reads the series back. Charted on [`desktop/src/lib/components/NetWorthChart.svelte`](apps/desktop/src/lib/components/NetWorthChart.svelte), reached from the Dashboard. | A snapshot is a fact about a date, not an editable value, so the table is an insert-only ledger with `RAISE(ABORT)` triggers and is registered in `LEDGER_TABLES` — two devices that captured the same account and date offline keep both facts and the read side picks the freshest, rather than one silently overwriting the other. Still not built: no asset/liability valuation beyond account balances, and no goals or peer comparison. |
| **Goals** | **Not built**<!-- parity absent="goal_create" --> | — | No goal table, type, endpoint or screen exists anywhere in the tree. |
...
| **Categorisation quality** | **Built**<!-- parity reachable="transaction_categorize" --> | Cascade on every inbound transaction (stored mapping → pack classifier) and the learning loop (`transaction_categorize` → `insert_correction` + `upsert_mapping`) in [`core/src/service.rs`](crates/slipscan-core/src/service.rs); merchant keys derived from bank narratives in [`core/src/util.rs`](crates/slipscan-core/src/util.rs); pack rules in [`packs/src/engine.rs`](crates/slipscan-packs/src/engine.rs); LLM/OCR slip extraction in [`crates/slipscan-extract/`](crates/slipscan-extract) | Two limits worth naming: a correction changes *future* rows only (nothing re-classifies transactions already imported), and correcting one is a desktop/HTTP action — the CLI has no `categorize` command, so a CLI-only user gets the automatic cascade and no way to teach it. |
| **Household / shared spending** | **Built**<!-- parity reachable="transaction_attribute,transaction_split_set,report_member_expense,report_member_contribution,report_member_category,report_settle_up" --> | [`core/src/migrations/0006_members.sql`](crates/slipscan-core/src/migrations/0006_members.sql), [`core/src/repo/member.rs`](crates/slipscan-core/src/repo/member.rs), attribution + splits + four member reports (`transaction_attribute`, `transaction_split_set`, `report_member_expense`, `report_member_contribution`, `report_member_category`, `report_settle_up`) in [`core/src/service.rs`](crates/slipscan-core/src/service.rs), screen at [`desktop/src/routes/Household.svelte`](apps/desktop/src/routes/Household.svelte), `slipscan member` / `attribute` / `split` / `report members` / `report settle-up` in [`cli/src/main.rs`](crates/slipscan-cli/src/main.rs), and the member routes in [`server/src/routes.rs`](crates/slipscan-server/src/routes.rs) — all three surfaces | Limit worth naming: members are local data with no logins and no access control — "sharing" means sharing the data folder or pointing clients at your own server. `member_capability_grant`/`_revoke` now exist for a future per-person authority model (docs/ROLES.md), but nothing enforces them yet and they are CLI-only. |
...
| **Fee tracking** | **Not built**<!-- parity absent="fee_detect" --> | — | No fee detection, classification or report anywhere; bank fees land as ordinary transactions and are visible only if some category happens to catch them. |
