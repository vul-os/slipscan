...
**How much of that is actually built?** [PARITY.md](PARITY.md) scores all 24 capabilities on both axes — Built / Partial / Not built, each with a file-level citation. The short version: the personal-finance loop is real (accounts, categorisation that learns, budgets, household attribution), and the accounting side is a ledger, not yet an accounting product — **there is no invoicing, and no quotes, bills, aged receivables, fixed assets or payroll**. Nothing in that document is rounded up.

SlipScan has also **absorbed [FlowStock](ROADMAP.md#phase-6--inventory--trade-the-flowstock-fold)**, a retired Vulos product for offline-first multi-branch inventory, on the reasoning that Xero and QuickBooks treat stock as a *module of the book* rather than a neighbouring app — a goods receipt is an inventory event and a journal entry, and two databases cannot both own that fact. Landed so far: `locations`, `contacts` and the product catalogue in the core. **None of it reaches a screen yet**, which is why those rows read Partial rather than Built. The honest near-term effect is that the Xero axis gets *longer* before it gets greener, because absorbing inventory adds capabilities to measure faster than it adds ones that are finished.
